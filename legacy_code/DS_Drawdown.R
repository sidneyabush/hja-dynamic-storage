...
discharge <- discharge %>%
  ...
  dplyr::mutate(
    Q_smoothed = rollmean(MEAN_Q, k = 7, fill = NA, align = "right")
  )

discharge <- discharge %>%
  dplyr::group_by(waterYear, SITECODE) %>%
  dplyr::mutate(
    hydro_flux = cumsum(MEAN_Q),
    hydro_flux_prop = hydro_flux / max(hydro_flux)
  )
...
discharge <- discharge %>%
  ...
  dplyr::mutate(
    tag = if_else(Q_smoothed == max(Q_smoothed), "peak", NA_character_)
  )
...
ggplot(last_peak, aes(wyd, SITECODE)) +
  geom_boxplot() +
  theme_bw() +
...
for (i in 1:length(years)) {
  ...
  p1 <- ggplot() +
    geom_line(
      dis_df,
      mapping = aes(get_waterYearDay(date), MEAN_Q, group = waterYear),
      alpha = 0.4
    ) +
    geom_point(
      last_peak_df,
      mapping = aes(wyd, last_peak_value),
      col = "blue"
    ) +
    theme_bw() +
...
setwd(
  "/Users/keirajohnson/Box Sync/05_Storage_Manuscript/03_Data/DynamicStorage"
)
...
ggplot(DS_compute, aes(get_waterYearDay(DATE), DS_sum)) +
  geom_line(aes(col = waterYear, group = waterYear)) +
  facet_wrap(~SITECODE) +
  labs(
    x = "Day of Water Year",
    y = "Dynamic Storage Drawdown (mm)",
    col = "Water Year"
  ) +
  theme_classic() +
