#!/usr/bin/env Rscript
# Merge and plot ET method diagnostics from support outputs.
# Inputs: OUT_MET_SUPPORT_DIR/daily_water_balance_all_ET_methods_1997_present.csv
# Outputs: EXPLORATORY_ET_METHODS_DIR/*png
...
# --- 2. SCATTERPLOTS (calibration window, 4 panels) ------
for(site in site_list){
  d <- df %>%
    filter(SITECODE == site,
           between(DATE, calibration_start, calibration_end))
...
p_grid_full <- ggplot(all_long, aes(DATE, ET_mm_day, color=Method)) +
  ...
  scale_color_manual(values=method_colors) +
  labs(title=sprintf("ET Methods Across All Sites (WY %d-%d)", WY_START, WY_END),
       x="Date", y="ET (mm/day)", color="Method") +
...
# --- 4a. PER-SITE CALIBRATION-WINDOW -------------------
for(site in site_list){
...
cal_grid <- all_long %>%
  filter(between(DATE, calibration_start, calibration_end))

p_grid_cal <- ggplot(cal_grid, aes(DATE, ET_mm_day, color=Method)) +
  geom_line(size=0.4, alpha=0.7) +
  facet_wrap(~SITECODE, scales="free_y", ncol=3) +
  scale_color_manual(values=method_colors) +
  labs(title=sprintf("ET Methods Across All Sites (Calibration Window: %s to %s)",
                     format(calibration_start, "%Y-%m-%d"),
                     format(calibration_end, "%Y-%m-%d")),
       x="Date", y="ET (mm/day)", color="Method") +
  theme_bw(base_size=12) +
  theme(strip.text       = element_text(face="bold", size=9),
        axis.text.x      = element_text(angle=45,hjust=1,size=7),
...
ggsave(file.path(plot_dir, "grid_all_sites_calibration_window.png"),
