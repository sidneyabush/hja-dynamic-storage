...
assert_unique_keys <- function(df, keys, df_name) {
  ...
  if (nrow(dupes) > 0) {
    stop(
      paste0(
        "Non-unique join keys detected in ",
        df_name,
        " for keys (",
        paste(keys, collapse = ", "),
        ")."
...
mtt_fyw <- read_csv(
  ...
  mutate(
    ...
    MTT2L_val = if ("MTT2L" %in% names(.)) {
      suppressWarnings(as.numeric(MTT2L))
    } else {
      NA_real_
    },
    MTT2H_val = if ("MTT2H" %in% names(.)) {
      suppressWarnings(as.numeric(MTT2H))
    } else {
      NA_real_
    },
    MTT2M_val = if ("MTT2M" %in% names(.)) {
      suppressWarnings(as.numeric(MTT2M))
    } else {
      NA_real_
    },
...
write.csv(HJA_avg, file.path(master_dir, MASTER_SITE_FILE), row.names = FALSE)
...
response_metrics <- c("T_7DMax", "Q_7Q5", "P_WetSeason")
...
storage_metric_cols <- STORAGE_METRIC_ORDER[
  STORAGE_METRIC_ORDER %in%
    c(dynamic_metrics, mobile_metrics_annual, extended_metrics)
]
annual_metric_cols <- intersect(
  c(storage_metric_cols, response_metrics),
  names(HJA_annual)
)
...
metric_site_counts <- tibble(metric = metric_cols) %>%
  ...
  mutate(
    ...
    sites = paste(
      site_metric_matrix$site[site_metric_matrix[[metric]]],
      collapse = ", "
    )
  ) %>%
  ungroup()
...
comprehensive_summary <- site_info %>%
  ...
  left_join(
    date_ranges %>% select(site, hydro_start, hydro_end, hydro_n_days),
    by = "site"
  )
...
eco_response_availability <- HJA_annual %>%
  ...
  mutate(
    ...
    missing_reason = case_when(
      site == "WS09" &
        n_wy_T_7DMax ==
          0 ~ "No WS09 stream-temperature records in HT00451_v10.txt",
...
eco_response_wy_coverage <- HJA_annual %>%
  mutate(site = as.character(site)) %>%
  select(site, year, T_7DMax, Q_7Q5, P_WetSeason) %>%
  pivot_longer(
    cols = c(T_7DMax, Q_7Q5, P_WetSeason),
    names_to = "response",
    values_to = "value"
  ) %>%
