...
manifest <- bind_rows(
  ...
  collect_files(file.path(OUTPUT_DIR, "stats"), "stat")
) %>%
  mutate(
    section = case_when(
      ...
      grepl("^stats/", rel_path) ~ "stats",
      TRUE ~ "other"
    ),
    path_group = case_when(
      grepl("anova_tukey", rel_path) ~ "anova_tukey",
      grepl("pca", rel_path) ~ "pca",
      grepl("mlr_catch_chars_storage|catch_chars_storage_mlr", rel_path) ~ "mlr_watershed_storage",
      grepl("mlr_storage_eco|storage_eco_mlr", rel_path) ~ "mlr_storage_eco",
...
coverage <- tribble(
  ...
  "mlr_watershed_storage", FALSE, TRUE, TRUE, TRUE,
  "mlr_storage_eco", FALSE, TRUE, TRUE, TRUE,
  ...
  mutate(
    ...
    has_supp_figure = case_when(
      analysis == "site_differences_anova_tukey" ~ has_file("figs/supp/stats/anova_tukey/"),
      analysis == "multivariate_pca" ~ has_file("figs/supp/stats/pca/pca_scree\\.(png|pdf)$"),
      analysis == "mlr_watershed_storage" ~ has_file("figs/supp/stats/mlr/catch_chars_storage_mlr_beta\\.(png|pdf)$"),
      analysis == "mlr_storage_eco" ~ has_file("figs/supp/stats/mlr/storage_eco_mlr_beta\\.(png|pdf)$"),
    ...
    has_table = case_when(
      ...
      analysis == "mlr_watershed_storage" ~ has_file("tables/mlr/catch_chars_storage_mlr_table\\.csv$"),
      analysis == "mlr_storage_eco" ~ has_file("tables/mlr/storage_eco_mlr_table\\.csv$"),
      TRUE ~ FALSE
    ),
    has_stat = case_when(
      analysis == "site_differences_anova_tukey" ~ has_file("stats/anova_tukey/anova_results\\.csv$"),
      analysis == "multivariate_pca" ~ has_file("stats/pca/pca_variance_explained\\.csv$"),
      analysis == "mlr_watershed_storage" ~ has_file("stats/mlr_catch_chars_storage/catch_chars_storage_mlr_summary_strict\\.csv$"),
      analysis == "mlr_storage_eco" ~ has_file("stats/mlr_storage_eco/storage_eco_mlr_summary_strict\\.csv$"),
      TRUE ~ FALSE
    ),
