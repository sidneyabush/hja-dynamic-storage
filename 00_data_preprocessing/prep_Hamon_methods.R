...
if (!file.exists(pt_file)) {
  stop("Missing PT ET timeseries file: ", pt_file)
}
...
monthly_coef_szilagyi <- cal_window_raw %>%
  ...
  group_by(SITECODE, MONTH) %>%
  summarise(
    coef_szilagyi_month = median(ET_PT_szilagyi / ET_Hamon_uncalibrated),
    .groups = "drop"
  )
...
global_coef_szilagyi <- mean(
  monthly_coef_szilagyi$coef_szilagyi_month,
  na.rm = TRUE
)
...
data <- data %>%
  ...
  mutate(
    ...
    ET_Hamon_pt_szilagyi_monthly = ifelse(
      is.na(ET_Hamon_uncalibrated),
      NA_real_,
...
regressions_szilagyi <- map(reg_sites_szilagyi, function(site) {
  df <- cal_window %>% filter(SITECODE == site)
  if (nrow(df) >= 10) {
    lm(ET_Hamon_pt_szilagyi_monthly ~ ET_PT_szilagyi, data = df)
  } else {
    NULL
  }
}) %>%
...
pred_szilagyi <- map_dfr(reg_sites_szilagyi, function(site) {
  ...
  if (!is.null(m) && nrow(df) > 0) {
    ...
    tibble(
      SITECODE = character(),
      DATE = as.Date(character()),
      ET_Hamon_pt_szilagyi_pred = double()
    )
  }
})
...
data <- data %>%
  mutate(
    ...
    ET_Hamon_pt_szilagyi_interpolated = coalesce(
      ET_Hamon_pt_szilagyi_monthly,
...
data <- data %>%
  ...
  mutate(
    ...
    ET_Hamon_pt_szilagyi_interp_full = na.approx(
      ET_Hamon_pt_szilagyi_interpolated,
      x = DATE,
      na.rm = FALSE
    ),
    ...
    ET_Hamon_pt_szilagyi_interp_full = na.locf(
      ET_Hamon_pt_szilagyi_interp_full,
      na.rm = FALSE
    ),
...
write_csv(
  ...
  file.path(
    output_dir,
    "daily_MET_water_balance_all_ET_methods_1997_present.csv"
  )
)
