...
unlink(file.path(
  output_dir,
  c(
    "catchment_char_storage_mlr_pca_loadings.csv",
    "catchment_char_storage_mlr_pca_variance_explained.csv",
    "catchment_char_storage_mlr_pca_scores_pc1_pc2.csv"
  )
))
...
features_kept <- names(feature_sds_vec)[
  is.finite(feature_sds_vec) & feature_sds_vec > 0
]

if (length(features_kept) == 0) {
  stop(
    "PCA failed: all candidate features are constant or missing after cleaning."
  )
}
...
write.csv(
  pca_df %>% select(site, year, PC1, PC2),
