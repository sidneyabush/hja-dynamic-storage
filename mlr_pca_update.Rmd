...
mlr1_selected <- mlr1_summary %>%
  ...
  transmute(
    ...
    r2_weighted_aicc_lte2 = round(R2_weighted_AICc_LTE2, 3),
...
```{r catch-beta-fig, out.width='75%', fig.cap='Watershed-characteristics standardized coefficients are Akaike-weighted model averages across all candidate models with delta AICc <= 2 for each response. Weights are exp(-0.5*delta AICc), normalized to sum to 1 within each response. Therefore, a predictor can show a non-zero cell even when it is not retained in the single minimum-AICc model, because it contributes through other near-competitive models. Use this figure for direction and relative effect size under model uncertainty; use the retained/dropped screening table for the exact predictor set of the single best model.'}
