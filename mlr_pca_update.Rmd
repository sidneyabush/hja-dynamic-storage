...
catch_coef <- read_csv(path("stats", "mlr_catch_chars_storage", "catch_chars_storage_mlr_results_strict.csv"), show_col_types = FALSE)
...
```{r catch-model-avg}
catch_avg_file <- path("stats", "mlr_catch_chars_storage", "catch_chars_storage_mlr_model_avg_coef.csv")
if (file.exists(catch_avg_file)) {
  cat("All watershed-model-averaged coefficients are saved here:\n\n")
  cat(paste0("- ", catch_avg_file, "\n"))
} else {
  kable(tibble(note = "Run `02_stats/catchment_storage_mlr.R` to generate catch_chars_storage_mlr_model_avg_coef.csv"))
}
```
...
```{r eco-pca-values}
eco_pca_var <- read_csv(path("stats", "pca", "eco_mlr_pca_variance_explained.csv"), show_col_types = FALSE) %>%
  mutate(cum_var = cumsum(Variance_Explained)) %>%
  mutate(across(c(Variance_Explained, cum_var), ~ round(.x, 4)))

eco_pca_load_file <- path("stats", "pca", "eco_mlr_pca_loadings.csv")
eco_pca_load <- read_csv(eco_pca_load_file, show_col_types = FALSE) %>%
  mutate(abs_pc1 = abs(PC1), abs_pc2 = abs(PC2))

kable(eco_pca_var, caption = "Eco-model PCA variance explained by component.")
cat("Eco-model PCA loadings file path:\n\n")
cat(paste0("- ", eco_pca_load_file, "\n"))
```

```{r eco-pca-figs, out.width='90%', fig.align='center'}
knitr::include_graphics(
  fig("supp", "stats", "pca", "eco_mlr_pca_biplot.png")
)
```

```{r eco-pca-screening, include=FALSE}
eco_screen <- read_csv(path("stats", "mlr_storage_eco", "storage_eco_mlr_pca_screening.csv"), show_col_types = FALSE)
eco_sel <- read_csv(path("stats", "mlr_storage_eco", "storage_eco_mlr_model_selection.csv"), show_col_types = FALSE)
...
best_sets <- eco_sel %>%
  ...
  ungroup() %>%
  select(Response, Candidate_Set)

eco_screen_best <- eco_screen %>%
  filter(Method == "strict") %>%
  inner_join(best_sets, by = c("Response", "Candidate_Set")) %>%
  mutate(
    pca_decision = ifelse(retained_after_pca, "retained", "dropped"),
    significant_pcs = if ("significant_pcs" %in% names(.)) significant_pcs else NA_character_,
    why_dropped = dplyr::case_when(
      retained_after_pca ~ NA_character_,
      !significant_any_pc ~ "not significant on retained PC(s)",
      drop_reason == "collinear_pair" ~ "dropped as collinear pair (abs(r) >= 0.7) with lower PCA score",
      TRUE ~ "dropped in PCA screening"
    )
  ) %>%
  transmute(
    response = Response,
  ...
  arrange(response, desc(pca_decision), predictor)
...
eco_out_file <- path("tables", "mlr", "storage_eco_mlr_aicc_lt2.csv")
...
```{r model-screening-eco}
hr <- "<hr style='border:0;border-top:2px solid #9aa4af;margin:0;'>"
eco_groups <- split(eco_screen_best, eco_screen_best$response)
...
kable(
  eco_with_sep,
  caption = "Eco-model PCA screening decisions for the best AICc candidate set for each response",
  escape = FALSE
)
```
