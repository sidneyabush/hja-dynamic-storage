...
The workflow reads inputs from `HJA_FINAL_WORKFLOW_ROOT/inputs` and writes outputs to `HJA_FINAL_WORKFLOW_ROOT/outputs` and `HJA_FINAL_WORKFLOW_ROOT/ms_materials`. Set `HJA_FINAL_WORKFLOW_ROOT` to the folder that stores the workflow data and manuscript materials.

## How To Review The Code

Start with `run_all.R`, which lists the full workflow in the order scripts are run. `config.R` stores shared paths, settings, site order, metric groups, and output folder names. `utils.R` stores shared functions, figure labels, plotting settings, input checks, output checks, hydrometeorological processing helpers, and model-selection helpers.
...
- `04_tables`: writes supporting information tables generated by the R workflow
...
- Figure 2: dynamic and extended-dynamic storage summaries and PCA
- Figure 3: mobile storage summaries
- Figure 4: catchment controls on storage metrics
- Figure 5: storage controls on ecological response variables
